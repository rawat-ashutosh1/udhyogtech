...
exports.listUsers = async (req, res) => {
  const [rows] = await pool.query(
    "SELECT id,name,email,role,is_active,mobile FROM users where role!='Job Seeker'"
  );
  res.json(rows);
};
...
exports.createCandidate = async (req, res) => {
    try {
        ...
        await pool.query(
            `INSERT INTO candidates (user_id) VALUES (?)`, [insertedId]
        );
        res.status(201).json({
            success: true,
...
exports.storeCandidate = async (req, res) => {
    try {
        const { id } = req.params;
        const { con_status, interview_stage, joining_cycle, post_joining, remarks } = req.body;
        await pool.query(
            `UPDATE candidates 
             SET con_status = ?, interview_stage = ?, joining_cycle = ?, post_joining = ?, remarks = ?
             WHERE user_id = ?`,
            [con_status, interview_stage, joining_cycle, post_joining, remarks, id]
        );
